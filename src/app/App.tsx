...
export function App() {
  ...
  const reserve = selected && world && selectedCount > 0 && afterTransit === 0
    ...
    : null;
  const canSend = active && selected?.lifecycle === 'active' && selected.riskStatus !== 'critical' && selectedCount > 0 && (afterTransit > 0 || Boolean(reserve));
  const canStabilize = active && selected?.lifecycle === 'active' && selected.riskStatus !== 'stable' && isImportantPortal(state, selected) && selected.stabilizationBonus < gameBalance.maxStabilizationBonus && state.cycle.stabilizationAttemptsUsed < gameBalance.stabilizationAttempts;
  const canObserve = active && selected?.lifecycle === 'active' && selected.riskStatus === 'dangerous' && isVeryImportantPortal(state, selected) && available.length > 0 && !state.employees.some((employee) => employee.role.type === 'observer') && energyAfterTransit(selected, 1, gameBalance) > 0;
  ...
  return (
    <div className={styles.page}>
      ...
      {state.activeView === 'worklog' ? <main className={styles.worklog}><h2>AI Worklog</h2><p>Проектирование и реализация ведутся человеком совместно с Codex. Человек определил игровой сценарий, ограничения и ключевые правила. AI подготовил документацию, доменную модель, состояние и интерфейс.</p><p>Подробный журнал работы и исправлений находится в <code>docs/AI_WORKLOG.md</code>. Фактическое время заполняет владелец; достоверная статистика токенов недоступна.</p></main> : <main>
        ...
        {state.storageWarning && <p role="alert" className={styles.alert}>{state.storageWarning}</p>}
        ...
        <div className={styles.mainGrid}>
          ...
          <section className={styles.detail} aria-label="Детали портала">{selected && world ? <><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Выбранный портал</p><h2>{selected.name}</h2></div><span className={styles.badge}>{selected.lifecycle === 'active' ? riskNames[selected.riskStatus] : lifecycleNames[selected.lifecycle]}</span></div><p>Ведёт в мир «{world.name}». {recommendationForPortal(selected)}</p><dl className={styles.facts}><div><dt>Энергия</dt><dd>{selected.energy.toFixed(1)} / 100</dd></div><div><dt>Риск</dt><dd>{Math.round(portalRisk(selected, gameBalance) * 100)}% · {riskNames[selected.riskStatus]}</dd></div><div><dt>До схлопывания</dt><dd>{duration(remainingLifetime(selected, gameBalance))}</dd></div><div><dt>Рассеивание / стабильность</dt><dd>{selected.dissipationCoefficient.toFixed(2)} / {selected.stability.toFixed(2)}</dd></div><div><dt>В мире</dt><dd>{inWorld.length} сотрудников</dd></div></dl><p className={styles.hint}>Риск — доля истёкшего расчётного времени жизни. При 50% портал опасен, при 80% — критичен. Коэффициенты меняются раз в минуту.</p>
            {selected.lifecycle === 'active' && <div className={styles.actions}><h3>Экспедиция</h3><label htmlFor="group-size">Размер группы</label><select id="group-size" value={Math.min(groupSize, Math.max(1, available.length))} onChange={(event) => setGroupSize(Number(event.target.value))} disabled={!active || available.length === 0}>{Array.from({ length: Math.min(gameBalance.maxExpeditionSize, Math.max(1, available.length)) }, (_, index) => index + 1).map((count) => <option key={count} value={count}>{count}</option>)}</select><p>Цена отправки: {transitCost(selected, selectedCount, gameBalance).toFixed(1)} энергии. После перехода: {afterTransit.toFixed(1)}. {afterTransit === 0 && selectedCount > 0 ? reserve ? `Резерв: ${reserve.name}.` : 'Надёжного резервного портала нет.' : `Оценка жизни после перехода: ${duration(remainingLifetime({ ...selected, energy: afterTransit }, gameBalance))}.`}</p><button className={styles.primary} onClick={send} disabled={!canSend}>Отправить сотрудников</button><button onClick={returnGroup} disabled={!active || inWorld.length === 0}>Вернуть всех ({inWorld.length}){selected.riskStatus === 'critical' ? ' · аварийно' : ''}</button><h3>Управление порталом</h3><p>Стабилизация: шанс {Math.round(stabilizationChance(selected, Boolean(observer), gameBalance) * 100)}%. Осталось попыток: {gameBalance.stabilizationAttempts - state.cycle.stabilizationAttemptsUsed}.</p><button onClick={() => dispatch({ type: 'stabilizePortal', portalId: selected.id })} disabled={!canStabilize}>Стабилизировать</button><button onClick={() => dispatch({ type: 'sendObserver', portalId: selected.id, employeeId: available[0]?.id ?? '' })} disabled={!canObserve}>Назначить наблюдателя</button><button onClick={close} disabled={!active || world.researchStatus !== 'explored'}>Закрыть портал</button></div>}
            {selected.lifecycle === 'collapsed' && <button onClick={close} disabled={!active}>Закрыть схлопнувшийся портал</button>}</> : <p className={styles.empty}>Выберите портал, чтобы увидеть риск, прогноз перехода и доступные действия.</p>}</section>
        </div>
