...
isReliableReserve =
  ...
  energyAfterReturn(reserve, employeesAlreadyInWorld + sentGroup) > 0 &&
...
Поиск другого маршрута включает критичный портал, если через него можно
вернуть больше сотрудников. Критичный статус сам по себе не запрещает возврат;
если энергии не хватает даже на одного сотрудника, действие отклоняется.
...
Даты цикла, событий и результатов проверяются на корректность до создания
`AppState`. Представление дат также имеет безопасный текст при неожиданном
невалидном значении.
