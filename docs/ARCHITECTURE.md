...
текущей партии, сохраняя `resultHistory`. Очистка истории — отдельный action с
подтверждением.
...
`close` для `lifecycle = active` разрешён только порталу в `explored`-мир. При
отсутствии сотрудников он сразу ставит `lifecycle = closed` и
`closedReason = manual`. При
наличии сотрудников чистый селектор требует другой надёжный портал для того же
мира с `energyAfterReturn > 0` и `remainingTime >= 60`; UI дополнительно требует
подтверждение. Закрытие портала, для которого назначен наблюдатель, сначала
